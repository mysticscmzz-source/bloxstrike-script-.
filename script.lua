-- Bloxstrike Aimbot + ESP + RGB GUI (Fixed)
...
local ESPObjects = {}
local ScreenGui -- declared early
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if humanoid and humanoid.Health > 0 then
                ...
                ESPObjects[player] = {Box = box, Name = nameTag, Tracer = tracer, Root = root}
            end
        end
    end
end
...
print("[Bloxstrike] Fixed - GUI should now appear")
